...
class ApiService {
  ...
  /// POST /api/donor/me/verification — multipart upload of the donor's ID
  /// (front, plus back when that ID type has one — see kNoIdBackTypes) and
  /// the five guided face-angle captures. Auto-checked and (currently)
  /// auto-approved server-side rather than queued for a human reviewer —
  /// see submitVerification's own doc comment in donorPortal.controller.js
  /// for exactly what is/isn't actually verified. Returns the resulting
  /// status.
  static Future<Map<String, dynamic>> submitVerification(
    ...
    if (extractedBirthdate != null) request.fields['extractedBirthdate'] = extractedBirthdate;
    if (extractedAddress != null) request.fields['extractedAddress'] = extractedAddress;
    ...
    for (final entry in facePosePaths.entries) {
      request.files.add(await http.MultipartFile.fromPath('face_${entry.key}', entry.value));
    }
